  ...
  module Ozone
    module Parser
      ...
      def self.parse_event_from(node)
        ...
        if node.xpath('//x:offer', 'x' => 'urn:xmpp:rayo:1').any?
          Connfu::Event::Offer.new(offer_params(node).merge(:call_id => call_id))
        elsif node.xpath('//x:success', 'x' => 'urn:xmpp:tropo:say:complete:1').any?
          Connfu::Event::SayComplete.new(:call_id => call_id)
        elsif node.type == :result
          if (ref = node.xpath('x:ref', 'x' => 'urn:xmpp:rayo:1').first)
        ...
        elsif node.xpath('//x:ringing', 'x' => 'urn:xmpp:rayo:1').any?
          Connfu::Event::Ringing.new(:call_id => call_id, :to => from, :from => to)
        elsif node.xpath('//x:answered', 'x' => 'urn:xmpp:rayo:1').any?
          Connfu::Event::Answered.new(:call_id => call_id)
        elsif node.xpath("//x:hangup", 'x' => "urn:xmpp:rayo:1").any?
          Connfu::Event::Hangup.new(:call_id => call_id)
        elsif stop = node.xpath('//x:stop', 'x' => 'urn:xmpp:rayo:ext:complete:1').first
          recording_element = node.xpath('//x:recording', 'x' => 'urn:xmpp:rayo:record:complete:1').first
          Connfu::Event::RecordingStopComplete.new(:call_id => call_id, :uri => recording_element.attributes['uri'].value)
        elsif complete = node.xpath('//x:success', 'x' => 'urn:xmpp:tropo:ask:complete:1').first
          captured_input = complete.xpath('//x:interpretation', 'x' => 'urn:xmpp:tropo:ask:complete:1').first.inner_text
      ...
      def self.transfer_complete?(node)
        Connfu::TransferState.event_map.each do |k, v|
          if node.xpath("//x:#{k}", 'x' => 'urn:xmpp:tropo:transfer:complete:1').any?
      ...
      def self.offer_params(node)
        ...
        offer = node.xpath('//x:offer', 'x' => 'urn:xmpp:rayo:1').first
        from = offer.xpath('x:header[@name="From"]', 'x' => 'urn:xmpp:rayo:1').first["value"]
        to = offer.xpath('x:header[@name="To"]', 'x' => 'urn:xmpp:rayo:1').first["value"]
