...
describe Connfu::Commands::Ask do
  ...
  describe "generating XMPP iq" do
    ...
    it "should generate ask iq" do
      subject.xpath("//x:ask", "x" => "urn:xmpp:tropo:ask:1").should_not be_empty
    end
    ...
    it "should set the mode to default dtmf" do
      subject.xpath("//x:ask", "x" => "urn:xmpp:tropo:ask:1").first.attributes["mode"].value.should eq "dtmf"
    end

    it "should set the terminator to hash by default" do
      subject.xpath("//x:ask", "x" => "urn:xmpp:tropo:ask:1").first.attributes["terminator"].value.should eq "#"
    end

    it "should include the prompt to be spoken as an element within the ask" do
      prompt_elements = subject.xpath("//x:ask/x:prompt", "x" => "urn:xmpp:tropo:ask:1")
    ...
    it "should include the number of digits in the choices element" do
      choices = subject.xpath("//x:ask/x:choices", "x" => "urn:xmpp:tropo:ask:1")
