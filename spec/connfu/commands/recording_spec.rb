...
describe Connfu::Commands::Recording do
  ...
  describe "generating XMPP iq for a Start command" do
    subject do
      Connfu::Commands::Recording::Start.new(
        :call_jid => 'call-jid', :from => 'client-address'
      ).to_iq
    end
  ...
  describe "generating XMPP iq for a Start command" do
    describe "with optional max length parameter" do
      subject do
        Connfu::Commands::Recording::Start.new(
          :call_jid => 'call-jid', :from => 'client-address', :max_length => 25000
        ).to_iq
      end
    ...
    describe "with optional beep parameter" do
      subject do
        Connfu::Commands::Recording::Start.new(
          :call_jid => 'call-jid', :from => 'client-address', :beep => false
        ).to_iq
      end
    ...
    describe "with recording format passed as :wav" do
      subject do
        Connfu::Commands::Recording::Start.new(
          :call_jid => 'call-jid', :from => 'client-address', :format => :wav
        ).to_iq
      end
    ...
    describe "with recording format passed as :gsm" do
      subject do
        Connfu::Commands::Recording::Start.new(
          :call_jid => 'call-jid', :from => 'client-address', :format => :gsm
        ).to_iq
      end
    ...
    describe "with recording format passed as :raw" do
      subject do
        Connfu::Commands::Recording::Start.new(
          :call_jid => 'call-jid', :from => 'client-address', :format => :raw
        ).to_iq
      end
    ...
    describe "with recording format passed as :mp3" do
      subject do
        Connfu::Commands::Recording::Start.new(
          :call_jid => 'call-jid', :from => 'client-address', :format => :mp3
        ).to_iq
      end
    ...
    describe "with recording format passed as unsupported format" do
      it 'should raise exception' do
        lambda {
          Connfu::Commands::Recording::Start.new(
            :call_jid => 'call-jid', :from => 'client-address', :format => :unknown_format
          ).to_iq
    ...
    describe "with recording format and supported format codec" do
      subject do
        Connfu::Commands::Recording::Start.new(
          :call_jid => 'call-jid', :from => 'client-address', :format => :wav, :codec => :mulaw_pcm_64k
        ).to_iq
      end
    ...
    describe "with recording format and unsupported format codec" do
      it 'should raise exception' do
        lambda {
          Connfu::Commands::Recording::Start.new(
            :call_jid => 'call-jid', :from => 'client-address', :format => :wav, :codec => :unknowncodec
          ).to_iq
    ...
    describe "passing format codec without specifying the format" do
      it 'should raise exception' do
        lambda {
          Connfu::Commands::Recording::Start.new(
            :call_jid => 'call-jid', :from => 'client-address', :codec => :mulaw_pcm_64k
          ).to_iq
  ...
  describe "generating XMPP iq for a Stop command" do
    subject do
      Connfu::Commands::Recording::Stop.new(:call_jid => 'call-jid', :from => 'client-address', :ref_id => 'abc123').to_iq
    end
