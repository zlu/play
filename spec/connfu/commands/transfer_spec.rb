...
describe Connfu::Commands::Transfer do
  ...
  describe "generating XMPP iq" do
    subject do
      ...
      Connfu::Commands::Transfer.new(:call_jid => 'call-jid', :from => 'client-address', :transfer_to => @transfer_to).to_iq
    end
    ...
    context 'with a timeout parameter' do
      subject do
        @transfer_to = ['sip:1324@connfu.com']
        Connfu::Commands::Transfer.new(:call_jid => 'call-jid', :from => 'client-address', :transfer_to => @transfer_to, :timeout => 5000).to_iq
      end
