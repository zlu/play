...
describe "answer and transfer a call" do
  class AnswerAndTransferExample
    ...
    on :offer do
      ...
      transfer_with_roundrobin(['sip:userb@127.0.0.1', 'sip:userc@127.0.0.1'])
    end
  end
  ...
  it "should send the transfer command only to the first end-point once the say command has completed" do
    run_fake_event_loop Connfu::Event::Offer.new(:from => @server_address, :to => @client_address),
